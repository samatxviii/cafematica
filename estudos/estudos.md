...
```mermaid
xychart-beta
    title "Escala de visualizações ligadas a aprendizagem e provas"
    x-axis ["TikTok ENEM no Brasil", "YouTube learning/how-to nos EUA"]
    y-axis "Visualizações em bilhões" 0 --> 6
    bar [3.8, 5.5]
```
...
```mermaid
xychart-beta
    title "Crescimento do conteúdo educacional no TikTok Brasil"
    x-axis ["Publicações", "Visualizações"]
    y-axis "Multiplicador" 0 --> 6.5
    bar [6, 5.6]
```
...
```mermaid
timeline
    title Janelas inferidas de maior demanda por conteúdo de Matemática no Brasil
    Fevereiro a março : inscrições da OBMEP e início de quadros de desafio
    Abril a maio : isenção do ENEM e preparação inicial
    Junho a agosto : primeira fase da OBMEP e reforço de meio de ano
    Setembro a outubro : segunda fase da OBMEP e sprint final para o ENEM
    Novembro : revisão final, correções e cortes de melhores respostas
```
